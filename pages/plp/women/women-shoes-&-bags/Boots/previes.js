  ...
  root.innerHTML = `
    ...
    <section class="ui-section">
      <div class="container">
        ...
        <section class="plp-header" id="plp-header"></section>
  ...
  <aside class="cart-drawer">
    ...
    <div class="cart-header">
      ...
      <button
        type="button"
        class="cart-close"
        aria-label="${i18n[lang].cartClose}">
        ✕
      </button>
...
function renderBreadcrumb() {
  ...
  breadcrumb.innerHTML = `
    <a href="#">${i18n[lang].breadcrumbHome}</a>
    <span>/</span>
    <a href="#">${i18n[lang].breadcrumbCollection}</a>
    <span>/</span>
    <span>${i18n[lang].breadcrumbPage}</span>
  `;
}

function renderHeader() {
  const header = document.getElementById("plp-header");
  ...
  header.innerHTML = `
    ...
    <span class="plp-header__count number">
      ${products.length} ${i18n[lang].products}
    </span>
  `;
}
...
function renderSearch() {
  ...
  input.addEventListener("input", (event) => {
    ...
    filteredProducts = products.filter((product) => {
      ...
      const badge = text(product.badge).toLowerCase();
  ...
  if (!list.length) {
    container.innerHTML = `
      <div class="product-grid__empty">
        <h3>${i18n[lang].noProducts}</h3>
        <p>${i18n[lang].tryAnotherKeyword}</p>
      </div>
    `;
...
function renderCart() {
  ...
  if (title) {
    title.textContent = i18n[lang].cartTitle;
  }

  if (checkout) {
    checkout.textContent = i18n[lang].checkout;
  }

  if (!cart.length) {
    body.innerHTML = `
      <div class="cart-empty">
        <p>${i18n[lang].emptyCart}</p>
      </div>
    `;

    if (totalBox) {
      totalBox.innerHTML = `
        <span>${i18n[lang].subtotal}</span>
  ...
  body.innerHTML = cart.map((product) => {
