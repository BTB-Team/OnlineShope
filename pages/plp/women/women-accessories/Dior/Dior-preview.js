...
import{language}from"./Dior-i18n.js";
let currentLanguage=localStorage.getItem("diorLanguage")||"en";
...
localStorage.setItem("diorLanguage",currentLanguage);
render();
}
...
<img class="product-card__image" src="${product.image}" alt="${currentLanguage==="fa"?product.nameFa||product.name:product.name}">
...
<a href="../pdp/pdp.html?brand=dior&id=${product.id}">${lang.details}</a>
</div>
</div>
</article>
`;
...
function getCart(){
try{
return JSON.parse(localStorage.getItem("cart_dior")||"[]");
}catch(error){
return[];
}
}
function saveCart(cart){
localStorage.setItem("cart_dior",JSON.stringify(cart));
}
