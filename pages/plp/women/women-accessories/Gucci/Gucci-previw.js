import{products}from"./Gucci-data.js";
import{language}from"./Gucci-i18n.js";
let currentLanguage=localStorage.getItem("gucciLanguage")||"en";
...
localStorage.setItem("gucciLanguage",currentLanguage);
render();
}
...
<a href="../pdp/pdp.html?brand=gucci&id=${product.id}">${lang.details}</a>
</div>
</div>
</article>
`;
...
function getCart(){
try{
return JSON.parse(localStorage.getItem("cart_gucci")||"[]");
}catch(error){
return[];
}
}
function saveCart(cart){
localStorage.setItem("cart_gucci",JSON.stringify(cart));
}
