// nail-tools-preview.js
// Preview logic for Nail Tools Collection with Shopping Cart

import { nailToolsProducts } from './nail-tools-data.js';
import { nailToolsTranslations } from './nail-tools-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...nailToolsProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('nail-tools-root');
...
function getCurrentTranslations() {
  return nailToolsTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    nailToolsTranslations[currentLanguage] ||
    nailToolsTranslations.en;

  filteredProducts =
    nailToolsProducts.filter(product => {
...
function renderBreadcrumb() {
  ...
  return `
    <nav class="breadcrumb">
      ...
      <span>/</span>
      <span>${t.breadcrumbNailTools}</span>
    </nav>
  `;
}
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = nailToolsProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Nail Tools PLP with Cart - Initializing...');

  if (!rootContainer) {
    console.error('❌ Root container #nail-tools-root not found!');
    return;
  }
  ...
  console.log('✅ Nail Tools PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', nailToolsProducts.length);
