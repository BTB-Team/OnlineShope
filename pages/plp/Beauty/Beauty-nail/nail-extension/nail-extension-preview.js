// nail-extension-preview.js
// Preview logic for Nail Extension Collection with Shopping Cart

import { nailExtensionProducts } from './nail-extension-data.js';
import { nailExtensionTranslations } from './nail-extension-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...nailExtensionProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('nail-extension-root');
...
function getCurrentTranslations() {
  return nailExtensionTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    nailExtensionTranslations[currentLanguage] ||
    nailExtensionTranslations.en;

  filteredProducts =
    nailExtensionProducts.filter(product => {
...
function renderBreadcrumb() {
  ...
  return `
    <nav class="breadcrumb">
      ...
      <span>/</span>
      <a href="#">${t.breadcrumbNail}</a>
      <span>/</span>
      <span>${t.breadcrumbNailExtension}</span>
    </nav>
  `;
}
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = nailExtensionProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Nail Extension PLP with Cart - Initializing...');

  if (!rootContainer) {
    console.error('❌ Root container #nail-extension-root not found!');
    return;
  }
  ...
  console.log('✅ Nail Extension PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', nailExtensionProducts.length);
