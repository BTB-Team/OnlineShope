// nail-equipment-preview.js
// Preview logic for Nail Equipment Collection with Shopping Cart

import { nailEquipmentProducts } from './nail-equipment-data.js';
import { nailEquipmentTranslations } from './nail-equipment-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...nailEquipmentProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('nail-equipment-root');
...
function getCurrentTranslations() {
  return nailEquipmentTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    nailEquipmentTranslations[currentLanguage] ||
    nailEquipmentTranslations.en;

  filteredProducts =
    nailEquipmentProducts.filter(product => {
...
function renderBreadcrumb() {
  ...
  return `
    <nav class="breadcrumb">
      ...
      <span>/</span>
      <span>${t.breadcrumbNailEquipment}</span>
    </nav>
  `;
}
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = nailEquipmentProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Nail Equipment PLP with Cart - Initializing...');

  if (!rootContainer) {
    console.error('❌ Root container #nail-equipment-root not found!');
    return;
  }
  ...
  console.log('✅ Nail Equipment PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', nailEquipmentProducts.length);
