// nail-care-preview.js
// Preview logic for Nail Care Collection with Shopping Cart

import { nailCareProducts } from './nail-care-data.js';
import { nailCareTranslations } from './nail-care-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...nailCareProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('nail-care-root');
...
function getCurrentTranslations() {
  return nailCareTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    nailCareTranslations[currentLanguage] ||
    nailCareTranslations.en;

  filteredProducts =
    nailCareProducts.filter(product => {
...
function renderBreadcrumb() {
  ...
  return `
    <nav class="breadcrumb">
      ...
      <span>/</span>
      <span>${t.breadcrumbNailCare}</span>
    </nav>
  `;
}
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = nailCareProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Nail Care PLP with Cart - Initializing...');

  if (!rootContainer) {
    console.error('❌ Root container #nail-care-root not found!');
    return;
  }
  ...
  console.log('✅ Nail Care PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', nailCareProducts.length);
