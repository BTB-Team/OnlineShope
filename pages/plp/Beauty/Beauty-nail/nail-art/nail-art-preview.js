// nail-art-preview.js
// Preview logic for Nail Art Collection with Shopping Cart

import { nailArtProducts } from './nail-art-data.js';
import { nailArtTranslations } from './nail-art-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...nailArtProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('nail-art-root');
...
function getCurrentTranslations() {
  return nailArtTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    nailArtTranslations[currentLanguage] ||
    nailArtTranslations.en;

  filteredProducts =
    nailArtProducts.filter(product => {
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = nailArtProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Nail Art PLP with Cart - Initializing...');

  if (!rootContainer) {
    console.error('❌ Root container #nail-art-root not found!');
    return;
  }
  ...
  console.log('✅ Nail Art PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', nailArtProducts.length);
