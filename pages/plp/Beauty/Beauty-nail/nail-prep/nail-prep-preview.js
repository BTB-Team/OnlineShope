// nail-prep-preview.js
// Preview logic for Nail Prep Collection with Shopping Cart

import { nailPrepProducts } from './nail-prep-data.js';
import { nailPrepTranslations } from './nail-prep-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...nailPrepProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('nail-prep-root');
...
function getCurrentTranslations() {
  return nailPrepTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    nailPrepTranslations[currentLanguage] ||
    nailPrepTranslations.en;

  filteredProducts =
    nailPrepProducts.filter(product => {
...
function renderBreadcrumb() {
  ...
  return `
    <nav class="breadcrumb">
      ...
      <span>/</span>
      <span>${t.breadcrumbNailPrep}</span>
    </nav>
  `;
}
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = nailPrepProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Nail Prep PLP with Cart - Initializing...');

  if (!rootContainer) {
    console.error('❌ Root container #nail-prep-root not found!');
    return;
  }
  ...
  console.log('✅ Nail Prep PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', nailPrepProducts.length);
