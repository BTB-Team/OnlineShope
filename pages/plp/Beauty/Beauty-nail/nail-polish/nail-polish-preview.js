// nail-polish-preview.js
// Preview logic for Nail Polish Collection with Shopping Cart

import { nailPolishProducts } from './nail-polish-data.js';
import { nailPolishTranslations } from './nail-polish-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...nailPolishProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('nail-polish-root');
...
function getCurrentTranslations() {
  return nailPolishTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    nailPolishTranslations[currentLanguage] ||
    nailPolishTranslations.en;

  filteredProducts =
    nailPolishProducts.filter(product => {
...
function renderBreadcrumb() {
  ...
  return `
    <nav class="breadcrumb">
      ...
      <span>/</span>
      <span>${t.breadcrumbNailPolish}</span>
    </nav>
  `;
}
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = nailPolishProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Nail Polish PLP with Cart - Initializing...');
  
  if (!rootContainer) {
    console.error('❌ Root container #nail-polish-root not found!');
    return;
  }
  ...
  console.log('✅ Nail Polish PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', nailPolishProducts.length);
