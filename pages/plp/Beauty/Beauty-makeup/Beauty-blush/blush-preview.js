// blush-preview.js
// Preview logic for Blush Collection with Shopping Cart

import { blushProducts } from './blush-data.js';
import { blushTranslations } from './blush-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...blushProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('beauty-blush-root');
...
function getCurrentTranslations() {
  return blushTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    blushTranslations[currentLanguage] ||
    blushTranslations.en;

  filteredProducts =
    blushProducts.filter(product => {
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = blushProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Blush PLP with Cart - Initializing...');
  
  if (!rootContainer) {
    console.error('❌ Root container #beauty-blush-root not found!');
    return;
  }
  ...
  console.log('✅ Blush PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', blushProducts.length);
