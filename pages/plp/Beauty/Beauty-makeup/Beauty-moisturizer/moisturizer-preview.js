// moisturizer-preview.js
// Preview logic for Moisturizer Collection with Shopping Cart

import { moisturizerProducts } from './moisturizer-data.js';
import { moisturizerTranslations } from './moisturizer-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...moisturizerProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('beauty-moisturizer-root');
...
function getCurrentTranslations() {
  return moisturizerTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    moisturizerTranslations[currentLanguage] ||
    moisturizerTranslations.en;

  filteredProducts =
    moisturizerProducts.filter(product => {
...
function renderBreadcrumb() {
  ...
  return `
    <nav class="breadcrumb">
      ...
      <span>/</span>
      <a href="#">${t.breadcrumbSkincare}</a>
      <span>/</span>
      <span>${t.breadcrumbMoisturizer}</span>
    </nav>
  `;
}
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = moisturizerProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Moisturizer PLP with Cart - Initializing...');
  
  if (!rootContainer) {
    console.error('❌ Root container #beauty-moisturizer-root not found!');
    return;
  }
  ...
  console.log('✅ Moisturizer PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', moisturizerProducts.length);
