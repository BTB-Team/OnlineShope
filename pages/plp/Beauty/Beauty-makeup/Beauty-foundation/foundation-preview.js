// foundation-preview.js
// Preview logic for Foundation Collection with Shopping Cart

import { foundationProducts } from './foundation-data.js';
import { foundationTranslations } from './foundation-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...foundationProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('beauty-foundation-root');
...
function getCurrentTranslations() {
  return foundationTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    foundationTranslations[currentLanguage] ||
    foundationTranslations.en;

  filteredProducts =
    foundationProducts.filter(product => {
...
function renderBreadcrumb() {
  ...
  return `
    <nav class="breadcrumb">
      ...
      <span>/</span>
      <span>${t.breadcrumbFoundation}</span>
    </nav>
  `;
}
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = foundationProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Foundation PLP with Cart - Initializing...');
  
  if (!rootContainer) {
    console.error('❌ Root container #beauty-foundation-root not found!');
    return;
  }
  ...
  console.log('✅ Foundation PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', foundationProducts.length);
