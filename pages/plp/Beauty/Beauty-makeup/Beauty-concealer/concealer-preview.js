// concealer-preview.js
// Preview logic for Concealer Collection with Shopping Cart

import { concealerProducts } from './concealer-data.js';
import { concealerTranslations } from './concealer-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...concealerProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('beauty-concealer-root');
...
function getCurrentTranslations() {
  return concealerTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    concealerTranslations[currentLanguage] ||
    concealerTranslations.en;

  filteredProducts =
    concealerProducts.filter(product => {
...
// ✅ تابع removeFromCart با Toast Notification
...
function openCart() {
  ...
  // ✅ تنظیم موقعیت Cart بر اساس زبان
  if (currentLanguage === 'fa') {
    // دری: Cart از سمت راست باز می‌شود
    cartDrawer.style.right = 'auto';
    cartDrawer.style.left = '0';
  } else {
    // انگلیسی: Cart از سمت چپ باز می‌شود
...
// ✅ تابع ارسال سفارش به واتساپ با شماره صحیح
function handleWhatsAppCheckout() {
  ...
  // ✅ شماره واتساپ صحیح
  const whatsappNumber = '93787708642'; // فرمت بین‌المللی: کد کشور افغانستان (93) + شماره
...
function renderBreadcrumb() {
  ...
  return `
    <nav class="breadcrumb">
      ...
      <span>/</span>
      <span>${t.breadcrumbConcealer}</span>
    </nav>
  `;
}
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = concealerProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Concealer PLP with Cart - Initializing...');
  
  if (!rootContainer) {
    console.error('❌ Root container #beauty-concealer-root not found!');
    return;
  }
  ...
  console.log('✅ Concealer PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', concealerProducts.length);
