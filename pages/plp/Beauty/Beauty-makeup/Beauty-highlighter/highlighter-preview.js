// highlighter-preview.js
// Preview logic for Highlighter Collection with Shopping Cart

import { highlighterProducts } from './highlighter-data.js';
import { highlighterTranslations } from './highlighter-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...highlighterProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('beauty-highlighter-root');
...
function getCurrentTranslations() {
  return highlighterTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    highlighterTranslations[currentLanguage] ||
    highlighterTranslations.en;

  filteredProducts =
    highlighterProducts.filter(product => {
...
function renderBreadcrumb() {
  ...
  return `
    <nav class="breadcrumb">
      ...
      <span>/</span>
      <span>${t.breadcrumbHighlighter}</span>
    </nav>
  `;
}
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = highlighterProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Highlighter PLP with Cart - Initializing...');
  
  if (!rootContainer) {
    console.error('❌ Root container #beauty-highlighter-root not found!');
    return;
  }
  ...
  console.log('✅ Highlighter PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', highlighterProducts.length);
