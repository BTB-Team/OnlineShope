// powder-preview.js
// Preview logic for Powder Collection with Shopping Cart

import { powderProducts } from './powder-data.js';
import { powderTranslations } from './powder-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...powderProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('beauty-powder-root');
...
function getCurrentTranslations() {
  return powderTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    powderTranslations[currentLanguage] ||
    powderTranslations.en;

  filteredProducts =
    powderProducts.filter(product => {
    ...
    <article class="product-card">
      ...
      <div class="product-card__content">
        ...
        <div class="product-card__details">
          <a href="../../../../pdp/pdp.html?id=${product.id}">${t.viewDetails} →</a>
        </div>
      </div>
    </article>
  `;
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = powderProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Powder PLP with Cart - Initializing...');
  
  if (!rootContainer) {
    console.error('❌ Root container #beauty-powder-root not found!');
    return;
  }
  ...
  console.log('✅ Powder PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', powderProducts.length);
