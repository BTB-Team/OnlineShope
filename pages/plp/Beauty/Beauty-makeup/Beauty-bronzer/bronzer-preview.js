// bronze-preview.js
// Preview logic for Bronze Collection with Shopping Cart

import { bronzeProducts } from './bronzer-data.js';
import { bronzeTranslations } from './bronzer-i18n.js';

// ==================== STATE ====================
let currentLanguage = localStorage.getItem('selectedLanguage') || 'en';
let searchQuery = '';
let filteredProducts = [...bronzeProducts];
let cart = JSON.parse(localStorage.getItem('cart')) || [];

// ==================== DOM ====================
const rootContainer = document.getElementById('beauty-bronze-root');
...
function getCurrentTranslations() {
  return bronzeTranslations[currentLanguage];
}
...
function filterProducts() {
  ...
  const translations =
    bronzeTranslations[currentLanguage] ||
    bronzeTranslations.en;

  filteredProducts =
    bronzeProducts.filter(product => {
...
function attachEvents() {
  ...
  document.querySelectorAll('.add-to-cart-btn').forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      const product = bronzeProducts.find(p => p.id === productId);
      if (product) {
        addToCart(product);
      }
    });
...
// ==================== INIT ====================
function init() {
  console.log('🚀 Bronze PLP with Cart - Initializing...');
  
  if (!rootContainer) {
    console.error('❌ Root container #beauty-bronze-root not found!');
    return;
  }
  ...
  console.log('✅ Bronze PLP with Cart rendered successfully!');
  console.log('📦 Total Products:', bronzeProducts.length);
